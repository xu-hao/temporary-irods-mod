...
namespace irods {
    ...
    get_irods_root_directory() {
        ...
        try {
            boost::filesystem::path path{dl_info.dli_fname[1] == '/'? dl_info.dli_fname+1:dl_info.dli_fname};
            path = boost::filesystem::canonical(path);
            path.remove_filename();
            path.remove_filename();
            path.remove_filename(); // Removes filename and the two directories (usr and lib) between libirods_common.so and base of irods install
            return path;
